...
import { adminUsers, programs, students, news, events, recruitments, contacts } from "../shared/schema.js";
...
async function migrate() {
  ...
  try {
    ...
    // Recruitments
    await db.schema.alterTable('recruitments', (table: any) => {
      table.varchar('url', { length: 500 });
    });
    // Insert sample recruitments
    for (const recruitment of sampleRecruitments) {
      await db.insert(recruitments).values({ ...recruitment, url: (recruitment as any).url || '' }).onDuplicateKeyUpdate({
        set: { ...recruitment, url: (recruitment as any).url || '' }
      });
    }
